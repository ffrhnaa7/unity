...
public enum GoblinState //finite state machine 
{
    Idle,
    Patrol,
    Chase,
...
public class GoblinAI : MonoBehaviour, IEnemy
{
    ...
    private void Awake()
    {
        ...
        navMeshAgent = GetComponent<NavMeshAgent>(); //finds route with navmesh
        currentHp = maxHp; //for health ( fight with player )
        player = GameObject.FindGameObjectWithTag("Player").transform; //finds player

        ChangeState(GoblinState.Idle); //from idle to ___
    }
